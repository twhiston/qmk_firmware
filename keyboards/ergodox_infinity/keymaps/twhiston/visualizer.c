...
static const uint32_t logo_background_color = LCD_COLOR(255, 0xFF, 0xFF);
static const uint32_t initial_color = LCD_COLOR(168, 0xFF, 0);

static const uint32_t led_emulation_colors[4] = {
    LCD_COLOR(0, 0, 0),
    LCD_COLOR(200, 60, 128),
    LCD_COLOR(168, 128, 10),
...
bool lcd_keyframe_display_layer_bitmap_tom(keyframe_animation_t* animation, visualizer_state_t* state) {
    ...
    gdispClear(White);
    gdispDrawString(0, 0, "twhiston", state->font_dejavusansbold12, Black);
...
void ergodox_right_led_4_off(void){
